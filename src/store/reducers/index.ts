...
import {
    ADD_WORKER, ActionAppReducerType, CALCULATE_SALARY,
    CHANGE_IS_ACTIVE_INFO_WINDOW, CHANGE_TOTAL_AMOUNT,
    DELETE_WORKER, RESET, SAVE_SHIFT_INFO, SET_FILTER_TYPE
} from '../actions';
...
export const appReducer = (state = initState, action: ActionAppReducerType) => {
    switch (action.type) {
        ...
        case DELETE_WORKER: {
            return { ...state, workers: state.workers.filter(el => el.id !== action.id) }
        }
