...
export type ActionAppReducerType =
  | ReturnType<typeof changeTotalAmountAC>
...
export const changeTotalAmountAC = (
  totalAmount: number,
  defaultSalary: number | string,
) => ({ type: CHANGE_TOTAL_AMOUNT, totalAmount, defaultSalary }) as const
export const addWorkerAC = (
  workerName: string,
  shifts: number,
  defaultSalary: number | string,
) => ({ type: ADD_WORKER, workerName, shifts, defaultSalary }) as const
export const deleteWorkerAC = (id: string) =>
  ({ type: DELETE_WORKER, id }) as const
export const calculateSalaryAC = () => ({ type: CALCULATE_SALARY }) as const
export const resetAC = () => ({ type: RESET }) as const
export const setFilterTypeAC = (filterType: FilterType) =>
  ({ type: SET_FILTER_TYPE, filterType }) as const
export const changeIsActiveInfoWindowAC = (isActiveInfoWindow: boolean) =>
  ({ type: CHANGE_IS_ACTIVE_INFO_WINDOW, isActiveInfoWindow }) as const
export const saveShiftInfoAC = (obj: {
  PER_WEEK: number
  PER_MONTH: number
  PER_YEAR: number
}) => ({ type: SAVE_SHIFT_INFO, obj }) as const
